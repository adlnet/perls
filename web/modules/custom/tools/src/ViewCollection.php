...
class ViewCollection implements CacheableDependencyInterface {
   ...
   *   An array of cache contexts for this viewcolelction.
   */
  public function __construct($itemsPerView = 10, $hidesEmptyViews = TRUE, array $cacheContexts = ['user']) {
    $this->itemsPerView = $itemsPerView;
    $this->hidesEmptyViews = $hidesEmptyViews;

    $this->cacheMetadata = new CacheableMetadata();

    $this->addCacheContext($cacheContexts);

  }

  /**
   * Add a cache context to this view Collection.
   *
   * @param array $cacheContexts
   *   An array of cache contexts to add.
   */
  public function addCacheContext(array $cacheContexts) {
   ...
   *   The langcode to limit this view to.
   *
   * @return ViewCollection
   *   The called object.
   */
  public function addViewById($viewId, array $args = NULL, $displayId = NULL, $langcode = NULL) {
    ...
    return $this->addView($view, $langcode);
  }
   ...
   *   The view to add.
   * @param array|string $langcode
   *   Optional, limit view to specific langcodes.
   *   If language filter is available.
   *
   * @return ViewCollection
   *   The called object.
   */
  public function addView(ViewExecutable $view, $langcode = NULL) {
    if (!$view) {
      throw new \Exception('A view must be defined');
    }
    // If langcode is set but is not an array make it one.
    if ($langcode && !is_array($langcode)) {
    ...
    foreach ($view->display_handler->getHandlers('filter') as $key => $handler) {
      ...
      // If langcode is set we override default language and limit filter
      // to supplied languages.
      if ($key == 'langcode' && $langcode) {
        $view->display_handler->handlers['filter'][$key]->value = $langcode;
      }
    }
  ...
  public function groupedResults() {
    ...
    foreach ($this->views as $view) {
      ...
      if ($restDisplayId) {
        $url = $view->hasUrl(NULL, $restDisplayId) ? $view->getUrl(NULL, $restDisplayId) : NULL;
      }
      else {
        $url = $view->hasUrl() ? $view->getUrl() : NULL;
      }
