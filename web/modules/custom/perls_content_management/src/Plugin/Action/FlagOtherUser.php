 ...
 * @Action(
 *   id = "flag_other_user",
 *   label = @Translation("Flag as an other user"),
 *   type = ""
 * )
 */
class FlagOtherUser extends ViewsBulkOperationsActionBase implements ViewsBulkOperationsPreconfigurationInterface, ContainerFactoryPluginInterface {
  ...
  public function execute($entity = NULL) {
    ...
    if ($user_id = $this->context['arguments'][0]) {
      ...
      elseif (isset($account)) {
        ...
        return $flag->getMessage('flag') ?: $this->t('@label added', ['@label' => $flag->label()]);
      }
    }
    else {
  ...
  public function buildPreConfigurationForm(array $form, array $values, FormStateInterface $form_state) {
    ...
    $form['flag'] = [
      '#title' => $this->t('Flags'),
      '#description' => $this->t('Select which flag will be added.'),
      '#type' => 'radios',
      '#options' => $flags_ids,
