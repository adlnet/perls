...
(function () {
  ...
  Annotator.Plugin.AnnotatorViewer = (function(_super) {
    ...
    AnnotatorViewer.prototype.events = {
      ...
      ".annotator-viewer-delete touchstart": "onDeleteClick",
      ".annotator-viewer-edit touchstart": "onEditClick",
    };
    ...
    function AnnotatorViewer(element, options) {
      ...
      $(element).append( this.createAnnotationPanel() );

      $(".container-anotacions").toggle();
      $("#annotations-panel").click(function(event) {
        if ($('#count-anotations').text() > 0) {
    ...
    */
    ...
    AnnotatorViewer.prototype.onDeleteClick = function(event) {
      ...
      if(confirm(i18n_dict.confirm_delete)) {
        this.click;
        var deleted = this.onButtonClick(event, 'delete');
        if ($('#count-anotations').text() == 0) {
          $(".container-anotacions").hide();
        }
        return deleted;
    ...
    AnnotatorViewer.prototype.onAnnotationCreated = function(annotation) {
      ...
      $(".annotations-list-uoc").removeClass("annotations-init");
    ...
    AnnotatorViewer.prototype.onAnnotationsLoaded = function(annotations) {
      ...
      if (annotations.length > 0) {
        ...
        $(".annotations-list-uoc").addClass("annotations-init");
      }
    ...
    AnnotatorViewer.prototype.onAnnotationDeleted = function(annotation) {
      ...
      annotationCount = $(".container-anotacions").find('.annotator-marginviewer-element').length;
      $('#count-anotations').text(annotationCount);
    ...
    AnnotatorViewer.prototype.mascaraAnnotation = function(annotation) {
      ...
      var delete_icon = "<div class=\"annotator-viewer-edit\" title=\"Edit\" style=\"float:right;margin-top:3px;\"></div><div class=\"annotator-viewer-delete\" title=\"" + i18n_dict.Delete + "\" style=\"float:right;margin-top:3px;margin-left:3px;\"></div>";

      if (annotation.estat==1 || annotation.permissions.read.length===0 ) {
        shared_annotation = "<div class=\"annotator-viewer-share\" title=\""+ i18n_dict.share +"\" style=\"margin:5px;float:left;\"></div>"
      }
      ...
      annotation_layer += '<div class="anotador_text">' + textAnnotation + '</div></div><div class="annotator-marginviewer-date">' + $.format.date(annotation.data_creacio, "MM/dd/yyyy HH:mm") + '</div><div class="annotator-marginviewer-quote">'+ annotation.quote + '</div><div class="annotator-marginviewer-footer"><span class="'+class_label+'">' + annotation.user + '</span>' + shared_annotation + delete_icon + '</div>';
     ...
     var myAnnotation=false;
      ...
      anotacioObject = $(annotation_layer).appendTo('.container-anotacions').click(function(event) {
        if (event.target.tagName.toLowerCase() === 'textarea') {
          // Avoid scrolling if the user is editing the annotation.
          return;
        }
        var viewPanelHeight = jQuery(window).height();
        var annotation_reference = annotation.id;

        $element = jQuery(`#${annotation_reference}`).length ? jQuery(`#${annotation_reference}`) : jQuery(`[data-annotation-id="${annotation_reference}"]`);
          if (!$element.length) {
          ...
          if ($element.length) {
            ...
            $('html, body').animate({
              scrollTop: $element.offset().top - (viewPanelHeight/2)
            }, 2000);
          }
      })
      .mouseover(function() {
